...
export async function deployContract(
  ...
  // TEMP console.info(`Deploying ${info} with ${argStr}`)
  ...
  // TEMP console.info(`... Completed! Contract ${nameStr}: ${res.address}`)
...
export async function findContract(name: string, cacheName?: string, address?: string) {
  cacheName ||= name;
  const nameStr = cacheName == name ? name : `${cacheName}(${name})`;
  // TEMP console.info(`Getting ${nameStr} from ${address || "cache"}`)
...
export async function getOrDeployContract(
  ...
  const nameStr = cacheName == name ? name : `${cacheName}(${name})`;
  // TEMP console.info(`Getting ${nameStr} from cache, deploy if not exist`)
...
export async function call<T>(
  ...
  if (txPromise instanceof Function) {
    ...
    // TEMP console.info(`Calling ${label}...`)
    const res = await txPromise;
    // TEMP console.info(`... Called! ${res}`)
    return res
  }
}
...
export async function reportGasUsed(provider: Provider, tx, label) {
  ...
  // TEMP console.info(label, gasUsed.toString())
  return gasUsed
}
